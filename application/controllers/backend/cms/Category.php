...
class Category extends CI_Controller {
	...
	{ 
        ...
        if ($this->upload->do_upload('cat_image')) {  
			...
			if($image){
				unlink($this->input->post('cat_image_old'));
			}
	...
	{  
		...
		$cat_posts = $this->db->select('cat_id')->from('web_posts')->where('cat_id', $cat_id)->get()->num_rows();
		$cat_cont = $this->db->select('cat_id')->from('web_article')->where('cat_id', $cat_id)->get()->num_rows();

		if ($cat_posts>0 || $cat_cont>0) {
			$this->session->set_flashdata('exception', "This Category isn't empty, it contains posts. Please delete the posts first");
