...
class Posts extends CI_Controller {
	...
	{  
        ...
        * Pagination Start
        ******************************/
        $config["base_url"] = base_url('backend/cms/posts/index');
        $config["total_rows"] = $this->db->count_all('web_posts');
        $config["per_page"] = 25;
        ...
        $data['article'] = $this->posts_model->read($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
        /******************************
        * Pagination ends
        ******************************/

		$data['content'] = $this->load->view("backend/posts/list", $data, true);
	...
	{ 
		...
		if (!empty($article_id)) {   
			...
			$this->form_validation->set_rules('headline_en', display('headline_en'),'required|is_unique[web_posts.headline_en]|max_length[255]');
        ...
        if ($this->upload->do_upload('article_image')) {  
			...
			if($image && $this->input->post('article_image_old')){
				unlink($this->input->post('article_image_old'));
			}

		}
		
		if($this->input->post('cat_id')){
			$cat_slug = $this->db->query("select slug FROM web_category WHERE cat_id ='" . $this->input->post('cat_id') . "'")->row()->slug;
		}
		...
		if ($this->form_validation->run()) 
		{
			...
			if (empty($article_id)) 
			{
				if ($this->posts_model->create($userdata)) {
				...
				redirect("backend/cms/posts/form/");

			} 
			else 
			{
				if ($this->posts_model->update($userdata)) {
				...
				redirect("backend/cms/posts/form/$article_id");
		...
		{ 
			$parent_cat = $this->db->select("cat_id")->from('web_category')->where('slug', 'posts')->get()->row();
			$child_cat = $this->db->select("*")->from('web_category')->where('parent_id', $parent_cat->cat_id)->get()->result();

			$data['child_cat'] = $child_cat;
			if(!empty($article_id)) {
				$data['title'] = "Edit Post";
				$data['article']   = $this->posts_model->single($article_id);

			}
		}
		
		$data['content'] = $this->load->view("backend/posts/form", $data, true);
		$this->load->view("backend/layout/main_wrapper", $data);
	...
	{  
		...
		$article = $this->posts_model->single($article_id);
		//delete its image from folder if it exist
		if ($article->article_image) {
			unlink($article->article_image);
		}

		if ($this->posts_model->delete($article_id)) {
